...
class ProductGallery extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('cat_id, product_id', 'numerical', 'integerOnly'=>true),
			array('image, title', 'length', 'max'=>250),
			array('temp1, temp2, created', 'length', 'max'=>50),
			array('description, update_time', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, image, title, description, cat_id, product_id, temp1, temp2, created, update_time', 'safe', 'on'=>'search'),
		);
	}
