<?php

/**
 * This is the model class for table "discount".
 *
 * The followings are the available columns in table 'discount':
 * @property integer $id
 * @property string $code
 * @property integer $percentage
 * @property integer $total_num
 * @property integer $used_num
 * @property integer $temp1
 * @property integer $temp2
 *
 * The followings are the available model relations:
 * @property Orders[] $orders
 */
class Discount extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return Discount the static model class
	 */
	public static function model($className=__CLASS__)
	{
		return parent::model($className);
	}

	/**
	 * @return string the associated database table name
	 */
	public function tableName()
	{
		return 'discount';
	}

	/**
	 * @return array validation rules for model attributes.
	 */
	public function rules()
	{
		// NOTE: you should only define rules for those attributes that
		// will receive user inputs.
		return array(
			array('code','unique'),
			array('percentage, total_num, used_num, temp1, temp2', 'numerical', 'integerOnly'=>true),
			array('code', 'length', 'max'=>255),

			array('code,percentage,total_num', 'required' ,'on'=>'create ,update' ),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, code, percentage, total_num, used_num, temp1, temp2', 'safe', 'on'=>'search'),
		);
	}

	/**
	 * @return array relational rules.
	 */
	public function relations()
	{
		// NOTE: you may need to adjust the relation name and the related
		// class name for the relations automatically generated below.
		return array(
			'orders' => array(self::HAS_MANY, 'Orders', 'discount_id'),
		);
	}

	/**
	 * @return array customized attribute labels (name=>label)
	 */
	public function attributeLabels()
	{
		return array(
			'id' => 'ID',
			'code' => 'Code',
			'percentage' => 'Percentage',
			'total_num' => 'Total Num',
			'used_num' => 'Number of used',
			'temp1' => 'Temp1',
			'temp2' => 'Temp2',
		);
	}

	/**
	 * Retrieves a list of models based on the current search/filter conditions.
	 * @return CActiveDataProvider the data provider that can return the models based on the search/filter conditions.
	 */
	public function search()
	{
		// Warning: Please modify the following code to remove attributes that
		// should not be searched.

		$criteria=new CDbCriteria;

		$criteria->compare('id',$this->id);
		$criteria->compare('code',$this->code,true);
		$criteria->compare('percentage',$this->percentage);
		$criteria->compare('total_num',$this->total_num);
		$criteria->compare('used_num',$this->used_num);
		$criteria->compare('temp1',$this->temp1);
		$criteria->compare('temp2',$this->temp2);

		return new CActiveDataProvider($this, array(
			'criteria'=>$criteria,
		));
	}

	public function getDiscount()
	{
		return CHtml::listData(Discount::model()->findAll(array('order'=>'id DESC')),'id','code');
	}
}